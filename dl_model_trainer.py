...
class DLModelTrainer:
    TARGET_COINS = ["BTC/KRW", "ETH/KRW", "XRP/KRW", "SOL/KRW", "DOGE/KRW", "AVAX/KRW", "LINK/KRW"]
    def __init__(self, model_path='price_predictor_dl.h5', scaler_path='price_scaler_dl.pkl', sequence_length=72):
    ...
    def load_historical_data(self, start_date_str: str, end_date_str: str):
        ...
        for ticker in ModelTrainer.TARGET_COINS: # ModelTrainer의 TARGET_COINS 사용
            ...
            if not os.path.exists(filepath_feather):
                print(f"- {ticker} 캐시 파일 ({filepath_feather})을 찾을 수 없습니다. 건너뜜.")
                continue
    ...
    def _build_lstm_model(self, input_shape):
        model = Sequential([
            LSTM(100, return_sequences=True, input_shape=input_shape), # 첫 번째 LSTM 층
    ...
    def train_model(self, historical_data: dict):
        ...
        for ticker, df_ohlcv in historical_data.items():
            ...
            feature_columns = [col for col in df_ohlcv.columns if col not in ['open', 'high', 'low', 'close', 'volume', 'future_close', 'price_change', 'label']]
            ...
            MIN_DATA_FOR_TRAINING = self.sequence_length + 1 # 시퀀스 생성에 필요한 최소 데이터 수
            if len(features_df) < MIN_DATA_FOR_TRAINING:
                print(f"Skipping {ticker}: Not enough data ({len(features_df)} rows) after cleaning. Minimum required: {MIN_DATA_FOR_TRAINING}")
                continue
            ...
            df_ohlcv['price_change'] = (df_ohlcv['future_close'] - df_ohlcv['close']) / df_ohlcv['close']
            ...
            labels = df_ohlcv['label'].loc[features_df.index].copy() # features_df와 인덱스 정렬
            ...
            if not features_df.empty and not labels.empty:
                ...
                print(f"Skipping {ticker}: Not enough data after feature/label generation. (Features empty: {features_df.empty}, Labels empty: {labels.empty})")
        ...
        # 모델 구축 및 학습
        self.model = self._build_lstm_model(input_shape=(X_train.shape[1], X_train.shape[2]))
        ...
        self.model.fit(X_train, y_train, epochs=100, batch_size=32, validation_data=(X_test, y_test), callbacks=callbacks, verbose=1)
    ...
    def predict_proba(self, df_ohlcv: pd.DataFrame) -> np.ndarray | None:
        ...
        processed_sequence = self.pipeline.process_for_prediction(df_ohlcv, self.sequence_length)
...
if __name__ == '__main__':
    ...
    for i in range(2): # 2 dummy coins
        ...
        df.drop(columns=['TP', 'MF', 'PMF', 'NMF', 'prev_TP', 'PMF_sum', 'NMF_sum', 'MFR'], inplace=True, errors='ignore')
    ...
    # 3. 모델 로드 및 예측
    if os.path.exists('lstm_price_predictor.h5') and os.path.exists('lstm_price_scaler.pkl'):
        predictor = DLModelTrainer()
        if predictor.load_model():
            sample_df = dummy_data['COIN0/KRW'].iloc[-100:] # 예측에 필요한 충분한 데이터
            buy_probability = predictor.predict(sample_df)
            if buy_probability is not None:
                print(f"Prediction for COIN0/KRW: Probability of Buy (label 1) in next 1 hour: {buy_probability:.4f}")
            else:

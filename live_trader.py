...
async def initialize(self):
    ...
    self._load_agents() # This will now find files
    self._init_analyzer()
...
async def main_live(): trader = LiveTrader(capital=1000000) await trader.initialize() await trader.run()

if name == 'main': try: asyncio.run(main_live()) except Exception as e: print('[FATAL] 봇이 최상위 레벨에서 중지되었습니다.') print(traceback.format_exc()) sys.exit(1) 
...
class MarketDiagnostics:
    ...
    async def get_volatility_index(self, symbol="BTC/USDT", days=30) -> float | None:
        ...
        try:
            ...
            ohlcv = await self.upbit_service.get_ohlcv(symbol_usdt, timeframe='1d', limit=days + 1)
            if ohlcv is None or ohlcv.empty:
                return None

            log_returns = np.log(ohlcv['close'] / ohlcv['close'].shift(1))
            annualized_volatility = log_returns.std() * np.sqrt(365) * 100 # 백분율로 표시
            print(f"  - [Diagnostics] 계산된 연율화 변동성: {annualized_volatility:.2f}%")
...
if __name__ == '__main__':
    ...
    async def main():
        ...
        print("\n--- 최종 시장 분석 요약 ---")
        print(f"  - 변동성 지수: {summary['volatility_index']:.2f}%" if summary['volatility_index'] is not None else "- 변동성 지수: 계산 실패")

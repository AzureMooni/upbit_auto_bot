...
class DataPipeline:
    ...
    def _load_scaler(self):
        ...
        try:
            if os.path.exists(self.scaler_path):
                ...
                print(
                    f"경고: 스케일러 파일({self.scaler_path})을 찾을 수 없습니다. 데이터 정규화 없이 진행됩니다."
                )
                return None
    ...
    def _generate_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["PMF"] = df.apply(
            lambda row: row["MF"] if row["TP"] > row["prev_TP"] else 0, axis=1
        )
        df["NMF"] = df.apply(
            lambda row: row["MF"] if row["TP"] < row["prev_TP"] else 0, axis=1
        )
        ...
        df.drop(
            columns=["TP", "MF", "prev_TP", "PMF", "NMF"], inplace=True, errors="ignore"
        )

        return df

    def process_for_prediction(
        self, df: pd.DataFrame, sequence_length: int = 72
    ) -> np.ndarray | None:
        """
        ...
        if len(features_df) < sequence_length:
            print(
                f"시퀀스 생성에 데이터가 충분하지 않습니다. 필요: {sequence_length}, 보유: {len(features_df)}"
            )
            return None
        ...
        if not all(col in latest_data.columns for col in self.feature_columns):
            print(
                "에러: 예측용 데이터에 스케일러가 필요로 하는 특징 컬럼이 일부 없습니다."
            )
            return None

...
def train_foundational_agent(
    ...
    if os.path.exists(log_dir):
        log(f"기존 로그 디렉토리 {log_dir}를 삭제합니다.")
        shutil.rmtree(log_dir)
    os.makedirs(log_dir, exist_ok=True)
    log("[STAGE 1/5] 완료.")

    log('[STAGE 2/5] 데이터 로딩 및 전처리 시작...')
    ...
    log(f'[STAGE 2/5] 완료. 전처리된 데이터 {data_path}에 저장 완료.')

    if not all_data_dict:
        log("오류: 전처리된 데이터가 없습니다. 훈련을 중단합니다.")
        return

    log("[STAGE 3/5] 데이터프레임 결합 및 스케일링 시작...")
    ...
    log(f"[STAGE 3/5] 완료. 스케일러가 다음 경로에 저장되었습니다: {scaler_save_path}")

    log("[STAGE 4/5] 강화학습(RL) 환경 설정 시작...")
    env = SimpleTradingEnv(df_scaled, lookback_window=50)
    env = FlattenObservation(env)
    vec_env = DummyVecEnv([lambda: env])
    log("[STAGE 4/5] 완료.")

    log("[STAGE 5/5] PPO 모델 훈련 시작...")
    model = PPO(
        "MlpPolicy",
        vec_env,
        verbose=1,
    ...
    log(f"모델 훈련을 시작합니다... (Total Timesteps: {total_timesteps})")
    model.learn(total_timesteps=total_timesteps)

    log(f"[STAGE 5/5] 완료. 모델을 다음 경로에 저장합니다: {model_save_path}")
    model.save(model_save_path)

    if not os.path.exists(stats_save_path):
        log(f'{stats_save_path}이(가) 없어 새로 생성합니다.')
        stats = {
        ...
        with open(stats_save_path, 'w') as f:
            json.dump(stats, f)
    log(f"기본 성과 파일 {stats_save_path} 생성 완료.")

...
class PortfolioManager:
    def __init__(self, upbit_service: UpbitService, allocated_capital: float, max_concurrent_trades: int):
        ...
        print(f"포트폴리오 매니저(주력 부대) 초기화 완료. 할당 자본: {self.total_capital:,.0f} KRW, 최대 동시 거래: {self.max_concurrent_trades}")
        ...
        while True:
            try:
                ...
                if dl_selected_ticker in self.active_trades:
                    print(f"주력 부대: {dl_selected_ticker}는 이미 활성 거래 중이므로 건너뜁니다.")
                    await asyncio.sleep(scan_interval_seconds)
                    continue

                # 4. RL 에이전트의 최종 승인
                print(f"주력 부대: DL 모델 선정 코인({dl_selected_ticker})에 대한 RL 에이전트의 최종 승인 확인 중...")
                ...
                if df_1h.empty or len(df_1h) < window_size:
                    print(f"주력 부대: {dl_selected_ticker}에 대한 RL 에이전트 평가용 데이터가 부족합니다 (필요: {window_size}, 현재: {len(df_1h)})." )
                    await asyncio.sleep(scan_interval_seconds)
                    continue
                ...
                if len(df_1h) < window_size:
                    print(f"주력 부대: {dl_selected_ticker}의 데이터가 전처리 후 너무 적어 평가할 수 없습니다.")
                    await asyncio.sleep(scan_interval_seconds)
                    continue
                ...
                if action != 1:  # 1: 매수
                    print(f"주력 부대: RL 에이전트가 {dl_selected_ticker}에 대한 매수를 승인하지 않았습니다 (액션: {action}).")
                    await asyncio.sleep(scan_interval_seconds)
                    continue
                ...
                # 5. 최종 거래 결정 및 실행
                print(f"✅ 주력 부대: 최종 승인에 따라 {dl_selected_ticker} 거래를 시작합니다.")
                
                # 할당된 자본 내에서 거래 자본 계산
                capital_for_trade = self.total_capital / (self.max_concurrent_trades - current_active_trades)

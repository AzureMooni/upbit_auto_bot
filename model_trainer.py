...
class ModelTrainer:
    ...
    def _create_labels(
        self, df: pd.DataFrame, look_forward_mins=5, threshold=0.003
    ) -> pd.DataFrame:
    ...
    def train_model(self):
        ...
        for ticker in self.target_coins:
            ...
            if os.path.exists(cache_path):
                ...
                print(
                    f"  경고: {ticker}의 캐시 파일을 찾을 수 없습니다. 전처리 단계를 먼저 실행하세요."
                )
        ...
        print(
            f"✅ 모델을 '{self.model_path}'에, 스케일러를 '{self.scaler_path}'에 저장했습니다."
        )

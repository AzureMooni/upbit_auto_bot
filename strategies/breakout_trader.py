...
class BreakoutTrader:
    def __init__(
        self, upbit_service: UpbitService, ticker: str, allocated_capital: float
    ):
        ...
        print(
            f"BreakoutTrader initialized for {self.ticker}. Allocated capital: {self.allocated_capital:,.0f} KRW."
        )
    ...
    async def run(self, interval_seconds: int = 60):  # Check every minute
        ...
        while True:
            try:
                ...
                ohlcv_daily_raw = await self.upbit_service.exchange.fetch_ohlcv(
                    self.ticker, "1d", limit=2
                )
                ...
                if not self.position_held:
                    ...
                    if current_price > (self.pp + self.breakout_value):
                        print(
                            f"[{time.strftime('%Y-%m-%d %H:%M')}] 🚀 Breakout UP detected for {self.ticker}. Attempting to BUY."
                        )
                        ...
                        if order and order.get("status") == "closed":
                            ...
                            print(
                                f"🟢 BUY executed for {self.ticker} at {self.entry_price:,.2f} KRW. TP: {self.r2:,.2f}, SL: {self.pp:,.2f}."
                            )
                        else:
                            print(
                                f"❌ BUY order failed or not closed for {self.ticker}."
                            )
                else:  # 포지션 보유 중: 익절 또는 손절 확인
                    # 익절 조건: 현재 가격이 R2에 도달
                    if current_price >= self.r2:
                        print(
                            f"[{time.strftime('%Y-%m-%d %H:%M')}] 🎉 Take Profit hit for {self.ticker} at {current_price:,.2f} KRW (R2: {self.r2:,.2f})."
                        )
                        balances = await self.upbit_service.get_all_balances()
                        amount_to_sell = balances.get(self.base_currency, {}).get(
                            "balance", 0
                        )
                        if amount_to_sell > 0:
                            ...
                            if order and order.get("status") == "closed":
                                print(
                                    f"🔴 SELL executed for {self.ticker} (Take Profit)."
                                )
                                self.position_held = False
                                return  # 전략 종료
                        else:
                            print(
                                f"Warning: No {self.base_currency} to sell for Take Profit."
                            )
                    ...
                    # 손절 조건: 현재 가격이 피봇 포인트 아래로 내려옴
                        ...
                        print(
                            f"[{time.strftime('%Y-%m-%d %H:%M')}] 🚨 Stop Loss hit for {self.ticker} at {current_price:,.2f} KRW (PP: {self.pp:,.2f})."
                        )
                        balances = await self.upbit_service.get_all_balances()
                        amount_to_sell = balances.get(self.base_currency, {}).get(
                            "balance", 0
                        ...
                        if amount_to_sell > 0:
                            ...
                            if order and order.get("status") == "closed":
                                print(
                                    f"🔴 SELL executed for {self.ticker} (Stop Loss)."
                                )
                                self.position_held = False
                                return  # 전략 종료
                        else:
                            print(
                                f"Warning: No {self.base_currency} to sell for Stop Loss."
                            )
                            self.position_held = False  # 포지션 상태 강제 초기화
                            return  # 전략 종료
                ...
                print(
                    f"An error occurred in BreakoutTrader run loop for {self.ticker}: {e}"
                )
...
UPBIT_SECRET_KEY=YOUR_SECRET_KEY""")
    ...
    async def test_breakout_trader():
        ...
        try:
            ...
            print(
                "BreakoutTrader example setup complete. To run, integrate into main.py and ensure API keys are set."
            )

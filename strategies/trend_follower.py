...
class TrendFollower:
    def __init__(
        ...
        print(
            f"TrendFollower initialized for {self.ticker} with dynamic trailing stop-loss (ATR period: {self.atr_period}). Allocated capital: {self.allocated_capital:,.0f} KRW."
        )
    ...
    async def _calculate_atr(self, ticker: str, exchange: ccxt.Exchange):
        ...
        """
        try:
            ohlcv_1h = await exchange.fetch_ohlcv(
                ticker, "1h", limit=self.atr_period + 10
            )  # ATR 계산에 필요한 충분한 데이터
            if not ohlcv_1h or len(ohlcv_1h) < self.atr_period:
                print(
                    f"Not enough OHLCV data for {ticker} to calculate ATR. (Need at least {self.atr_period}, got {len(ohlcv_1h) if ohlcv_1h else 0})"
                )
                return None
        ...
        while True:
            try:
                ...
                if not self.position_held:
                    print(
                        f"No position held. Attempting to buy {self.ticker} at market price."
                    )

                    order = await self.upbit_service.create_market_buy_order(
                        self.ticker, self.allocated_capital
                    )
                    ...
                    if order and order["status"] == "closed":  # 주문이 체결되었다면
                        ...
                        atr_value = await self._calculate_atr(
                            self.ticker, self.upbit_service.exchange
                        )
                        if atr_value is not None:
                            self.trailing_stop_price = current_price - (
                                self.atr_multiplier * atr_value
                            )
                            print(
                                f"🟢 매수 완료: {self.ticker} at {self.entry_price:,.2f} KRW. Dynamic Trailing Stop-Loss set at {self.trailing_stop_price:,.2f} KRW ({self.atr_multiplier} * ATR: {self.atr_multiplier * atr_value:,.2f})."
                            )
                        else:
                            print(
                                f"❌ ATR 계산 실패. 트레일링 스탑 설정 불가. {self.ticker} 재시도..."
                            )
                            self.position_held = (
                                False  # ATR 계산 실패 시 포지션 잡지 않음
                            )
                    ...
                    if current_price > self.high_water_mark:
                        ...
                        if atr_value is not None:
                            new_trailing_stop = self.high_water_mark - (
                                self.atr_multiplier * atr_value
                            )
                            if (
                                ...
                                print(
                                    f"📈 고점 업데이트: {self.high_water_mark:,.2f} KRW. Dynamic Trailing Stop-Loss: {self.trailing_stop_price:,.2f} KRW ({self.atr_multiplier} * ATR: {self.atr_multiplier * atr_value:,.2f})."
                                )
                        else:
                    ...
                    if current_price <= self.trailing_stop_price:
                        print(
                            f"🚨 트레일링 스탑 로스 발동! {self.ticker} 전량 시장가 매도."
                        )
                        ...
                        if amount_to_sell > 0:
                            ...
                            if order and order["status"] == "closed":
                                print(
                                    f"🔴 매도 완료: {self.ticker} 전량 시장가 매도. 손실/수익 확정."
                                )
                                self.position_held = False
                            else:
                            ...
                            print(
                                f"Warning: No {self.base_currency} to sell for trailing stop-loss."
                            )
...
UPBIT_SECRET_KEY=YOUR_SECRET_KEY""")
    ...
    async def test_trend_follower():
        ...
        try:
            ...
            print(
                "TrendFollower example setup complete. To run, integrate into main.py and ensure API keys are set."
            )

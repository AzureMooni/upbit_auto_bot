...
class GridTrader:
    def __init__(
        ...
        self.order_amount_per_grid = (
            self.allocated_capital / self.grid_count
        ...
        print(
            f"GridTrader initialized for {self.ticker} with {self.grid_count} grids from {self.lower_price} to {self.upper_price}. Allocated capital: {self.allocated_capital:,.0f} KRW, Order per grid: {self.order_amount_per_grid:,.0f} KRW"
        )
    ...
    async def _place_order(self, order_type: str, price: float):
        ...
        if order_type == "buy":
            ...
            try:
                order = await self.upbit_service.exchange.create_limit_buy_order(
                    symbol, amount, price
                )
                print(
                    f"Placed BUY order: {amount:.4f} {base_currency} at {price} {quote_currency}. Order ID: {order['id']}"
                )
                return order
            ...
            amount = (
                self.order_amount_per_grid / price
            )  # 이 부분은 실제 매도 가능 수량으로 대체되어야 함
            try:
                order = await self.upbit_service.exchange.create_limit_sell_order(
                    symbol, amount, price
                )
                print(
                    f"Placed SELL order: {amount:.4f} {base_currency} at {price} {quote_currency}. Order ID: {order['id']}"
                )
                return order
            except Exception as e:
...
UPBIT_SECRET_KEY=YOUR_SECRET_KEY""")
    ...
    async def test_grid_trader():
        ...
        try:
            ...
            print(
                "GridTrader example setup complete. To run, uncomment 'grid_trader.run()' and ensure API keys are set."
            )

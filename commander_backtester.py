...
class CommanderBacktester:
    ...
    def _precompute_indicators(self, df: pd.DataFrame, volatility_days=30, rsi_period=14) -> pd.DataFrame:
        """백테스팅에 필요한 모든 지표를 사전에 일괄 계산합니다."""
        print("  - 백테스팅에 필요한 거시 지표를 사전 계산 중...")
        df['daily_return'] = df['close'].pct_change()
        df['volatility'] = df['daily_return'].rolling(window=volatility_days).std() * np.sqrt(365) * 100
        df['rsi'] = _manual_rsi(df['close'], period=rsi_period)
        df.dropna(inplace=True)
        print("  - 지표 계산 완료.")
        return df

    def _simulate_main_squad_pnl(self, capital: float, market_return: float, alpha: float = 1.2) -> float:
        """주력 부대의 일일 손익을 시장 수익률 기반 프록시로 계산합니다."""
        return capital * market_return * alpha

    def _simulate_scalping_squad_pnl(self, capital: float, intraday_data: pd.DataFrame, trade_amount: float) -> float:
        ...
        for i in range(1, len(df)):
            ...
            if not position_held:
                if current_row['EMA_5'] > current_row['EMA_10'] and prev_row['EMA_5'] <= prev_row['EMA_10']:
                    position_held = True
                    entry_price = current_row['close']
            else:
                if current_row['close'] >= entry_price * take_profit_ratio or current_row['close'] <= entry_price * stop_loss_ratio:
    ...
    def run_simulation(self):
        ...
        for today in timeline:
            ...
            pnl_scalping = self._simulate_scalping_squad_pnl(scalping_squad_capital, intraday_data, trade_amount=50000)
        ...
        total_return = (report_df['portfolio_value'].iloc[-1] / self.initial_capital - 1) * 100

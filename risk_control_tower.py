...
class RiskControlTower:
    ...
    def check_mdd_circuit_breaker(self, portfolio_history: pd.Series) -> bool:
        ...
        if current_mdd < self.mdd_threshold:
            print(
                f"🚨 비상! 포트폴리오 최대 낙폭({current_mdd:.2%})이 임계값({self.mdd_threshold:.2%})을 초과했습니다!"
            )
            print(
                "🚨 서킷 브레이커를 발동하여 모든 거래를 중단하고 포지션을 청산합니다."
            )
            return True
        return False

    def determine_investment_size(
        self,
        win_rate: float,
        avg_profit: float,
        avg_loss: float,
        ...
        print(
            f"  - [RCT] 켈리 비율: {kelly_fraction:.4f}, 예측 확신도: {prediction_confidence:.4f}, 감성 팩터: {sentiment_factor:.4f}"
        )

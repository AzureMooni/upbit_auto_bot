...
class UpbitService:
    def __init__(self):
        ...
        if not self.access_key or not self.secret_key:
            raise ValueError(
                "UPBIT_ACCESS_KEY and UPBIT_SECRET_KEY must be set in config/.env"
            )
    ...
    async def get_balance(self):  # Made async
        ...
        return {"KRW": krw_balance, "coins": coin_balances}
    ...
    async def get_total_capital(self):  # Made async
        ...
        for currency, amount in balances["coins"].items():
            ...
            if current_price:
                ...
                print(
                    f"Warning: Could not get current price for {ticker}. Excluding from total capital calculation."
                )
    ...
    async def create_market_sell_order(self, symbol: str, amount: float):  # Made async
        ...
        try:
            ...
            print(
                f"Placed MARKET SELL order: {amount:.4f} {symbol.split('/')[0]} at market price. Order ID: {order['id']}"
            )
            return order
        except Exception as e:
            print(
                f"Error placing MARKET SELL order for {symbol} with amount {amount}: {e}"
            )
            return None

    async def create_market_buy_order(
        self, symbol: str, amount_krw: float
    ):  # Made async
        """
        ...
        try:
            ...
            print(
                f"Placed MARKET BUY order: {amount_krw:,.0f} KRW worth of {symbol.split('/')[0]} at market price. Order ID: {order['id']}"
            )
            return order
        except Exception as e:
            print(
                f"Error placing MARKET BUY order for {symbol} with amount_krkrw {amount_krw}: {e}"
            )
            return None
...
UPBIT_SECRET_KEY=YOUR_SECRET_KEY""")
    ...
    async def test_upbit_service():
        try:
            ...
            # 최신 OHLCV 데이터 가져오기
            latest_ohlcv = await upbit_service.fetch_latest_ohlcv(
                "BTC/KRW", "1h", limit=2
            )

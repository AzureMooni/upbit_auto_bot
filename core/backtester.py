...
class Backtester:
    ...
    def _fetch_ohlcv_data(self):
        ...
        """
        print(
            f"Fetching OHLCV data for {self.ticker} from {self.start_date} to {self.end_date}..."
        )
        ...
        # 시작일과 종료일 범위에 맞게 필터링
        df = df[
            (df["datetime"] >= self.start_date)
            & (df["datetime"] <= self.end_date + " 23:59:59")
        ]
    ...
    def run_test(
        ...
        for i, data in enumerate(self.ohlcv_data):
            ...
            if last_close_price is None:
                ...
                portfolio_values.append(
                    krw_balance + coin_balance * current_close_price
                )
                continue
            ...
            for grid_price in grids:
                ...
                if last_close_price >= grid_price and current_close_price < grid_price:
                    if (
                        grid_status[grid_price] != "buy"
                    ):  # 해당 그리드에서 아직 매수하지 않았다면
                        if krw_balance >= order_amount_krw:
                            ...
                            bought_positions.append(
                                {"price": current_close_price, "amount": amount_to_buy}
                            )
                            trade_count += 1
                            grid_status[grid_price] = "buy"  # 매수 완료 표시
                            print(
                                f"[{current_date}] BUY at {current_close_price:,.2f} (Grid: {grid_price:,.2f}). Coin: {coin_balance:.4f}, KRW: {krw_balance:,.0f}"
                            )
                        # else:
                ...
                # 가격이 그리드 라인 위로 상향 돌파했을 때 (매수 그리드 초기화)
                    ...
                    last_close_price < grid_price and current_close_price >= grid_price
                ):
                    if grid_status[grid_price] == "buy":
                        grid_status[grid_price] = (
                            None  # 매수 그리드 초기화 (다시 매수 가능)
                        )
            ...
            for grid_price in grids:
                ...
                if last_close_price <= grid_price and current_close_price > grid_price:
                    if (
                        grid_status[grid_price] != "sell"
                    ):  # 해당 그리드에서 아직 매도하지 않았다면
                        if len(bought_positions) > 0:
                            ...
                            profit = (
                                sell_price - position_to_sell["price"]
                            ) * amount_to_sell
                            ...
                            if profit > 0:
                                win_trades += 1
                            print(
                                f"[{current_date}] SELL at {sell_price:,.2f} (Grid: {grid_price:,.2f}). Profit: {profit:,.2f} KRW. Coin: {coin_balance:.4f}, KRW: {krw_balance:,.0f}"
                            )
                        # else:
        ...
        final_price = (
            self.ohlcv_data[-1]["close"] if self.ohlcv_data else last_close_price
        )
        if coin_balance > 0 and final_price is not None:
            total_bought_cost = sum(
                pos["price"] * pos["amount"] for pos in bought_positions
            )
            ...
            if total_bought_amount > 0:
                ...
                profit_remaining = (
                    final_price - avg_buy_price_remaining
                ) * coin_balance
                ...
                if profit_remaining > 0:
                    win_trades += 1
                print(
                    f"[END] Final SELL at {final_price:,.2f}. Profit from remaining: {profit_remaining:,.2f} KRW. Final KRW: {krw_balance:,.0f}"
                )
            else:  # 남은 코인이 있지만, bought_positions에 기록되지 않은 경우 (예: 초기 자본으로 코인 보유 시작)
                krw_balance += final_price * coin_balance
                coin_balance = 0.0
                print(
                    f"[END] Liquidated remaining coin at {final_price:,.2f}. Final KRW: {krw_balance:,.0f}"
                )

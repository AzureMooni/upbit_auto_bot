...
class TradingEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        # 포트폴리오 상태 추가
        portfolio_state = np.array([self.cash, self.asset_holdings * self.df.loc[self.df.index[self.current_step], 'close']], dtype=np.float32)
    ...
    def render(self, mode='human', close=False):
        if mode == 'human':
            info = self._get_info()
            print(f"Step: {self.current_step}, Total Asset: {info['total_asset']:.2f}, Cash: {info['cash']:.2f}, Holdings: {info['asset_holdings']:.6f}")